...
class BankPaymentVoucherComponent extends Component
{
    ...
    public function getCheckbookSerial()
    {
        // dd($this->data);
        $checkbook = Checkbook::find($this->data['cheque_id']);
        $start = 1;
        $end = $checkbook->pages;
        ...
        // Prefix 'd-' to each number and store in a new array
        $this->checks = array_map(function ($number) {
            $checkbook = Checkbook::find($this->data['cheque_id']);
            $voucher = Voucher::where('cheque_no', $checkbook->format . $number)->first();
            if (!$voucher) {
                return $checkbook->format . $number;
            }
        }, $numbers);
    ...
    {   
        $this->show = CommonController::checkPermission();
        $this->accounts = ChartOfAccount::where('active', 1)
            ->where('level', 3)
            ->where('account_type', 'bank');

        if ($this->show) {
            $this->accounts->whereIn('show', $this->show);
        }

        $this->accounts = $this->accounts->get();
        $this->banks = Checkbook::all();
    }

    public function render()
    {
        return view('livewire.account.bank-payment-voucher-component');
    }
    ...
    public function store()
    {
        ...
        if ($debitAmountsSum != (float)$this->data['credit_amount']) {
        ...
        try {
            ...
            $exception = DB::transaction(function () {
                ...
                // Create the voucher detail for the main account
                $voucherDetail = new VoucherDetail();
                $voucherDetail->voucher_id = $voucher->id;
                $voucherDetail->account_id = $this->data['account_from'];
                $voucherDetail->credit_amount = (float)$this->data['credit_amount'];
                $voucherDetail->active = $this->data['active'];
                $voucherDetail->save();

                // Create voucher details for each debit entry
                foreach ($this->debit_amounts as $item) {
